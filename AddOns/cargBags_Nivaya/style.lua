...
function MyContainer:OnContentsChanged()
	...
	for _,v in ipairs(buttonIDs) do
        ...
        local xPos = col * (38+4)
        local yPos = (-1 * row * (38+4)) - yPosOffs
	...
	self.ContainerHeight = (row + (col > 0 and 1 or 0)) * (38+4)
			...
			for j = 1,tNumSlots do
				...
				if tLink then
					if (strsub(tLink, 13, 21) == "battlepet") then
						...
						local _,tStackCount = GetContainerItemInfo(i,j)
						if cB_KnownItems[tName] and tStackCount then
							cB_KnownItems[tName] = cB_KnownItems[tName] + tStackCount
						else
							cB_KnownItems[tName] = tStackCount
						end
...
local createTextButton = function (name, parent, width, height)
	...
	button:SetScale(1/cBnivCfg.scale)
	M.CreateBG(button, 0)
	M.CreateGradient(button)
	
	local text = button:CreateFontString(nil, "OVERLAY")
	text:SetFont(M.media.font, 11, "THINOUTLINE")
	text:SetPoint("CENTER", 1, 0)
	text:SetText(name)

	button:HookScript("OnEnter", function(self) text:SetTextColor(1, .1, .1) end)
	button:HookScript("OnLeave", function(self) text:SetTextColor(1, 1, 1) end)
	return button
end
...
function MyContainer:OnCreate(name, settings)
	...
	self:SetWidth((38+4)*self.Columns)
	...
	local background = M.CreateBG(self)
	local shadow = M.CreateSD(self)
	background:SetScale(1/cBnivCfg.scale)
	background:SetPoint("TOPLEFT", -7, 5)
	background:SetPoint("BOTTOMRIGHT", 4, -4)
	shadow:SetScale(1/cBnivCfg.scale)
	shadow:SetPoint("TOPLEFT", -10, 8)
	shadow:SetPoint("BOTTOMRIGHT", 7, -7)
	...
	if(caption) then
        ...
        if tBag or tBank then
            local close = createTextButton("X", self, 15, 15)
    ...
    local tBtnOffs = 0
  	if tBag or tBank then
        ...
        self.bagToggle = createTextButton("B", self, 15, 15)
        ...
        if tBag and cBnivCfg.NewItems then
            local resetBtn = createTextButton("R", self, 15, 15)
            resetBtn:SetPoint("TOPRIGHT", self, "TOPRIGHT", -20-tBtnOffs, 1)
    ...
    if (tBag or tBank) then
		...
		fs:SetFont(M.media.font, 14, "OUTLINE")
    ...
    if tBag then
		...
		caption:SetPoint("BOTTOMRIGHT", infoFrame, -4, 7)
    ...
    self:SetScript('OnMouseUp', function(self, mouseButton)
        if (mouseButton == 'RightButton') and (IsAltKeyDown()) then
            ...
            if tID then 
                ...
                XToggleDropDownMenu(1, nil, cbNivCatDropDown, self, 0, 0)
            end
        end
    end)
end
