...
local PetBattleStartInfo = function(self, event, ...)
...
--------------------		鼠标提示
local PetBattleUnitTooltip_UpdateForUnitHook=function(self, petOwner, petIndex)
	if HPetSaves.Contrast then
--~ 一个额外的鼠标提示
		local speciesID=C_PetBattles.GetPetSpeciesID(petOwner,petIndex)
...
--------------------		UnitFrame上色
local PetBattleUnitFrame_UpdateDisplayHook=function(self)
...
------------------------一些小修改，可能会被暴雪更改(已经删掉了几个，就是暴雪修复的)---------------------------------
...
--~ 	生命
	...
	elseif power > health/5 and power > speed then
--~ 	攻击
		self.PowerFrame.power:SetText(format("%d(+%.2f)",power,power/level))
	elseif speed > health/5 and speed > power then
--~ 	速度
...
--~ 点击未收集的宠物链接直接链接到宠物日志
local FloatingBattlePet_ShowHook=function(speciesID,level)
	if level==0 then
...
--~ 修复宠物对战中一些快捷键失效的问题
local PetBattleFrame_ButtonUpHook=function(id)
...
SlashCmdList["PETQUALITY"] = function(msg, editbox)
	...
	if command =="" then
		if GetLocale()=="zhTW" then
			...
			DEFAULT_CHAT_FRAME:AddMessage("[/hpq sg]或者[/hpq 成长值]顯示成長值(最高的那一項)".."\n已經|cffffff00["..(HPetSaves.ShowGrowInfo and "開啟" or "關閉").."]|r",0,1,1)
			...
			DEFAULT_CHAT_FRAME:AddMessage("[/hpq sg]或者[/hpq 成长值]显示成长值(最高的那一项)".."\n已经|cffffff00["..(HPetSaves.ShowGrowInfo and "开启" or "关闭").."]|r",0,1,1)
	...
	if command =="sg" or string.find(command,"成长值") then
...
--~ local HPetBattleAny = CreateFrame("frame");
HPetBattleAny:SetScript("OnEvent", function(self, event, ...)
	if event == "ADDON_LOADED" then
		local name , arg1 = ...
		if name== "HPetBattleAny" then
			local loaded, reason = LoadAddOn("Blizzard_PetJournal")
			if loaded then
				HPetBattleAny:UnregisterEvent("ADDON_LOADED")
				PetBattlePrimaryUnitTooltip:HookScript("OnHide",function(self,...) GameTooltip:Hide() end)
				hooksecurefunc("PetBattleUnitTooltip_UpdateForUnit",PetBattleUnitTooltip_UpdateForUnitHook)
				hooksecurefunc("PetBattleUnitFrame_UpdateDisplay",PetBattleUnitFrame_UpdateDisplayHook)
				PetBattleAbilityButton_OnEnter=PetBattleAbilityButton_OnEnterHook
				hooksecurefunc("PetJournal_UpdatePetCard",PetJournal_UpdatePetCardHook)
				hooksecurefunc("FloatingBattlePet_Show",FloatingBattlePet_ShowHook)
				hooksecurefunc("PetBattleFrame_ButtonUp",PetBattleFrame_ButtonUpHook)
			end
		end
	end
	if event == "PET_BATTLE_OPENING_START" then
		PetBattleStartInfo(self, event, ...)
	end
end)
HPetBattleAny:RegisterEvent("PET_BATTLE_OPENING_START");
