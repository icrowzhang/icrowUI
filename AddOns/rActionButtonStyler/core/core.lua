...
local function updatehotkey(self, actionButtonType)
	...
	key = replace(key, '(Middle Mouse)', 'B3')
	key = replace(key, '(Mouse Wheel Down)', 'WD')
	key = replace(key, '(Mouse Wheel Up)', 'WU')
...
local function styleActionButton(bu)
    ...
    if cfg.hotkeys.show then
		ho:SetFont(cfg.font, cfg.hotkeys.fontsize, "MONOCHROMEOUTLINE")
		ho:ClearAllPoints()
		...
		if ho:GetText() then updatehotkey(bu) end
    else
		ho:Hide()
    end
	
    if cfg.macroname.show then
		na:SetFont(cfg.font, cfg.macroname.fontsize, "MONOCHROMEOUTLINE")
		na:ClearAllPoints()
