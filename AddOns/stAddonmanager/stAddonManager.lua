...
-- MEDIA & CONFIG ------------------------------------
------------------------------------------------------
local font = { ChatFontNormal:GetFont(), 13, "OUTLINE" }
...
function stAddonManager:LoadProfileWindow()
	...
	local bg = CreateFrame("Frame", nil, window)
	bg:SetPoint("TOPLEFT", window, "BOTTOMLEFT", 0, -10)
	bg:SetSize(175, 55)
	F.SetBD(bg)
		...
		for profileName, addonList in sort(stAddonProfiles, function(a, b) return strlower(b) > strlower(a) end) do
			...
			overlay.delete:SetScript("OnClick", function(self)
				if IsShiftKeyDown() then
					...
					print(L.ButtonText..": "..L.Confirm_Delete)
				end
			end)
			i = i + 1
		end
...
gmbAddOns:SetText(L.ButtonText)
F.Reskin(gmbAddOns)
