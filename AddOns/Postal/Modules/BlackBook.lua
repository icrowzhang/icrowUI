...
function Postal_BlackBook:OnEnable()
	if not Postal_BlackBookButton then
		...
		F.ReskinArrow(Postal_BlackBookButton, "right")
		Postal_BlackBookButton:SetPoint("LEFT", SendMailNameEditBox, "RIGHT", 5, 0)
...
function Postal_BlackBook.BlackBookMenu(self, level)
	...
	elseif level == 2 then
		info.notCheckable = 1
		if XUIDROPDOWNMENU_MENU_VALUE == "recent" then
			local realm = GetRealmName()
			local faction = UnitFactionGroup("player")
			local player = UnitName("player")
			local db = Postal.db.profile.BlackBook.recent
		...
		elseif XUIDROPDOWNMENU_MENU_VALUE == "alt" then
		...
		elseif XUIDROPDOWNMENU_MENU_VALUE == "friend" then
			-- Friends list
		...
		elseif XUIDROPDOWNMENU_MENU_VALUE == "guild" then
	...
	elseif level >= 3 then
		info.notCheckable = 1
		if XUIDROPDOWNMENU_MENU_VALUE == "deletealt" then
			local db = Postal.db.global.BlackBook.alts
			local realm = GetRealmName()
			local faction = UnitFactionGroup("player")
			local player = UnitName("player")
		...
		elseif strfind(XUIDROPDOWNMENU_MENU_VALUE, "fpart") then
			local startIndex = tonumber(strmatch(XUIDROPDOWNMENU_MENU_VALUE, "fpart(%d+)")) * 25 - 24
		...
		elseif strfind(XUIDROPDOWNMENU_MENU_VALUE, "gpart") then
			local startIndex = tonumber(strmatch(XUIDROPDOWNMENU_MENU_VALUE, "gpart(%d+)")) * 25 - 24
...
function Postal_BlackBook.ModuleMenu(self, level)
	...
	if level == 1 + self.levelAdjust then
		...
		XUIDropDownMenu_AddButton(info, level)
		local listFrame = _G["XDropDownList"..level]
	...
	elseif level == 2 + self.levelAdjust then
		...
		if XUIDROPDOWNMENU_MENU_VALUE == "AutoComplete" then
