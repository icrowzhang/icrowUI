...
function Postal_OpenAll:OnEnable()
	...
	if not Postal_OpenAllMenuButton then
		...
		Postal_OpenAllMenuButton:SetPoint("LEFT", button, "RIGHT", 5, 0);
		F.ReskinArrow(Postal_OpenAllMenuButton, "right")
...
function Postal_OpenAll.ModuleMenu(self, level)
	...
	elseif level == 2 + self.levelAdjust then
		...
		if XUIDROPDOWNMENU_MENU_VALUE == "AHMail" then
		...
		elseif XUIDROPDOWNMENU_MENU_VALUE == "NeutralAHMail" then
			info.text = L["Open all Auction cancelled mail"]
		...
		elseif XUIDROPDOWNMENU_MENU_VALUE == "NonAHMail" then
			info.text = L["Open all mail with attachments"]
			info.arg2 = "Attachments"
			info.checked = db.Attachments
			XUIDropDownMenu_AddButton(info, level)

		elseif XUIDROPDOWNMENU_MENU_VALUE == "OtherOptions" then
	...
	elseif level == 3 + self.levelAdjust then
		if XUIDROPDOWNMENU_MENU_VALUE == "KeepFreeSpace" then
