...
function Postal_Express.SetEnableAltClick(dropdownbutton, arg1, arg2, checked)
	...
	local i, j = string.match(dropdownbutton:GetName(), "XDropDownList(%d+)Button(%d+)")
	j = tonumber(j) + 1
	if checked then
		_G["XDropDownList"..i.."Button"..j]:Enable()
		_G["XDropDownList"..i.."Button"..j.."InvisibleButton"]:Hide()
	else
		_G["XDropDownList"..i.."Button"..j]:Disable()
		_G["XDropDownList"..i.."Button"..j.."InvisibleButton"]:Show()
	end
end
