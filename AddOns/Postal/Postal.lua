...
Postal_DropDownMenu.HideMenu = function()
	if XUIDROPDOWNMENU_OPEN_MENU == Postal_DropDownMenu then
		XCloseDropDownMenus()
	end
end
...
function Postal:OnInitialize()
	...
	F.ReskinArrow(Postal_ModuleMenuButton, "left")
	Postal_ModuleMenuButton:SetPoint("TOPRIGHT", -24, -4)
	Postal_ModuleMenuButton:SetScript("OnClick", function(self, button, down)
		...
		XToggleDropDownMenu(1, nil, Postal_DropDownMenu, self:GetName(), 0, 0)
	end)
	...
	if TOC < 40000 and not IsAddOnLoaded("!BlizzBugsSuck") then
		hooksecurefunc("XUIDropDownMenu_CreateFrames", Postal.FixMenuFrameLevels)
	end
...
function Postal.Menu(self, level)
	...
	elseif level == 2 then
		if XUIDROPDOWNMENU_MENU_VALUE == "OpenSpeed" then
		...
		elseif XUIDROPDOWNMENU_MENU_VALUE == "ChatOutput" then
		...
		elseif XUIDROPDOWNMENU_MENU_VALUE == "Profile" then
			-- Profile stuff
			info.hasArrow = 1
			info.keepShownOnClick = 1
			info.func = self.UncheckHack
		...
		elseif type(XUIDROPDOWNMENU_MENU_VALUE) == "table" and XUIDROPDOWNMENU_MENU_VALUE.ModuleMenu then
			-- Submenus for modules
			self.levelAdjust = 1
			XUIDROPDOWNMENU_MENU_VALUE.ModuleMenu(self, level)
			self.levelAdjust = 0
			self.module = XUIDROPDOWNMENU_MENU_VALUE
		end

	elseif level == 3 then
		if XUIDROPDOWNMENU_MENU_VALUE == "SetProfile" then
		...
		elseif XUIDROPDOWNMENU_MENU_VALUE == "CopyProfile" or XUIDROPDOWNMENU_MENU_VALUE == "DeleteProfile" then
			local cur = Postal.db:GetCurrentProfile()
			Postal.db:GetProfiles(t)
			table.sort(t)
			info.func = Postal.ProfileFunc
			info.arg1 = XUIDROPDOWNMENU_MENU_VALUE
			info.notCheckable = 1
			for i = 1, #t do
				local s = t[i]
				if s ~= cur then
